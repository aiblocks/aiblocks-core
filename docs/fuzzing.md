...
fuzzer with aiblocks-core. Support for this is still preliminary but it has
...
perform well in area (a) and with simple modifications we made to aiblocks-core, (b)
too. However, we've spent more energy with (c) for the `tx` fuzz mode, modifying
aiblocks-core such that it is more deterministic -- caches are cleared and randomness
...
## Building an instrumented aiblocks-core
...
to `overlay` and then `make fuzz`; this will do the following:
  ...
  - Run `aiblocks-core gen-fuzz fuzz-testcases/fuzz$i.xdr` ten thousand times
  ...
  - Run `afl-fuzz` on `aiblocks-core fuzz`, using those corpus directories
...
by feeding it to an instance of `aiblocks-core fuzz` run by hand, elsewhere (in
particular, not fighting for control over tmpdirs with the fuzzer's
`aiblocks-core` instances). Often a fuzzer "crash" is just the subprocess hitting
...
`aiblocks-core` in a different directory with `--enable-asan`, or valgrind, in
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Try limiting the instrumentation to `aiblocks-core` itself, not libsodium,
  ...
  - Make startup-modes at different points in the process: instantiate an
    ...
    `AiBlocksMessage`s. Let the fuzzer generte bucket ledger entries, and try to
