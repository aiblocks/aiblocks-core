...
TEST_CASE("pathpayment", "[tx][pathpayment]")
{
    ...
    SECTION("path payment destination amount 0")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance1 - 2 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{dlo, minBalance}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("dest amount too big for DLO")
    {
        auto a = root.create("a", minBalance1);
        for_versions_to(10, *app, [&] {
            REQUIRE_THROWS_AS(root.pay(a, dlo, 20, dlo,
    ...
    SECTION("path payment DLO with not enough funds")
    {
        auto market = TestMarket{*app};
        // see https://github.com/aiblocks/aiblocks-core/pull/1239
        auto minimumAccount =
            root.create("minimum-account", minBalanceNoTx + 2 * txfee + 20);
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(
                minimumAccount.pay(root, dlo, txfee + 21, dlo, txfee + 21, {}),
                ex_PATH_PAYMENT_STRICT_RECEIVE_UNDERFUNDED);
            // clang-format off
            market.requireBalances(
                {{minimumAccount, {{dlo, minBalanceNoTx + txfee + 20}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment asset with not enough funds")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{minimumAccount, {{dlo, minBalance1 - 2 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
            ...
            // clang-format off
            market.requireBalances(
                {{minimumAccount, {{dlo, minBalance1 - 3 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment source does not have trustline")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{noSourceTrust, {{dlo, minBalance - txfee}, {idr, 0}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
            ...
            // clang-format off
            market.requireBalances(
                {{noSourceTrust, {{dlo, minBalance - 2 * txfee}, {idr, 0}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment source is not authorized")
    {
        ...
        auto sourceNotAuthorized = [&]() {
            ...
            // clang-format off
            market.requireBalances(
                {{noAuthorizedSourceTrust, {{dlo, minBalance1 - 2 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
            ...
            // clang-format off
            market.requireBalances(
                {{noAuthorizedSourceTrust, {{dlo, minBalance1 - 3 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        };
            ...
            "paths")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance1 - 2 * txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment destination does not have trustline")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {noDestinationTrust, {{dlo, minBalance}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
    ...
    SECTION("path payment destination is not authorized")
    {
        ...
        auto destNotAuthorized = [&]() {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {noAuthorizedDestinationTrust, {{dlo, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
    ...
    SECTION("path payment destination line full")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, 10}, {usd, 0}}}});
            // clang-format on
    ...
    SECTION("path payment destination line overflow")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {idr, INT64_MAX - 10}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(source.pay(destination, idr, 11, idr, 11, {}),
                              ex_PATH_PAYMENT_STRICT_RECEIVE_LINE_FULL);
            // clang-format off
    ...
    SECTION("path payment not enough offers for first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance1 - 2 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment crosses own offer for first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance3 - 4 * txfee}, {cur1, 10}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
    ...
    SECTION("path payment crosses own offer for middle exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 5 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
    ...
    SECTION("path payment crosses own offer for last exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 5 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
            ...
            "first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
            ...
            "middle exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "last exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
    ...
    SECTION("path payment over send max DLO")
    {
        auto market = TestMarket{*app};
        auto source = root.create("source", minBalance);
        auto destination = root.create("destination", minBalance);
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(source.pay(destination, dlo, 10, dlo, 11, {}),
                              ex_PATH_PAYMENT_STRICT_RECEIVE_OVER_SENDMAX);
            market.requireBalances(
                {{source, {{dlo, minBalance - txfee}, {idr, 0}, {usd, 0}}},
                 {destination, {{dlo, minBalance}, {idr, 0}, {usd, 0}}}});
        });
    }
    ...
    SECTION("path payment over send max with real path")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
        });
    ...
    SECTION("path payment to self DLO")
    {
        auto market = TestMarket{*app};
        auto account = root.create("account", minBalance + txfee + 20);

        for_versions_to(7, *app, [&] {
            auto offers = account.pay(account, dlo, 20, dlo, 20, {});
            auto expected = std::vector<ClaimOfferAtom>{};
            REQUIRE(offers.success().offers == expected);
            market.requireBalances({{account, {{dlo, minBalance}}}});
        });

        for_versions_from(8, *app, [&] {
            account.pay(account, dlo, 20, dlo, 20, {});
            market.requireBalances({{account, {{dlo, minBalance + 20}}}});
        });
    }
    ...
    SECTION("path payment crosses destination offer for first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance1 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{dlo, minBalance4 - 4 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance1 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{dlo, minBalance4 - 4 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance1 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {destination, {{dlo, minBalance4 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 10}}}});
            // clang-format on
        });
    }
    ...
    auto usesWholeBestOffer = [&](bool testAuthorizedToMaintainLiabilities) {
        ...
        SECTION("path payment uses whole best offer for first exchange")
        {
            ...
            for_all_versions(*app, [&] {
                ...
                // clang-format off
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                    {mm12a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 20}, {cur2, 30}, {cur3, 0}, {cur4, 0}}},
                    {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 60}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                    {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                    {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                    {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
        }
        ...
        SECTION("path payment uses whole best offer for second exchange")
        {
            ...
            for_all_versions(*app, [&] {
                ...
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                    {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                    {mm23a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 30}, {cur3, 5}, {cur4, 0}}},
                    {mm23a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 30}, {cur3, 5}, {cur4, 0}}},
                    {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                    {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
        ...
        SECTION("path payment uses whole best offer for last exchange")
        {
            ...
            for_all_versions(*app, [&] {
                ...
                market.requireBalances(
                    ...
                    {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                    {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                    {mm34a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 4}, {cur4, 8}}},
                    {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 16}, {cur4, 2}}},
                    {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
        }
    ...
    SECTION("path payment reaches limit for offer for first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 4}, {cur2, 38}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 76}, {cur2, 2}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
    ...
    SECTION("path payment reaches limit for offer for second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0},  {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 4}, {cur3, 18}, {cur4, 0}}},
                 {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 36}, {cur3, 2}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 2}, {cur4, 9}}},
                 {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 18}, {cur4, 1}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    ...
    SECTION("path payment missing trust line for offer for first exchange")
    {
        ...
        SECTION("missing selling line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12a, {{dlo, minBalance3 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
        ...
        SECTION("missing buying line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
        ...
        SECTION("missing selling line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23a, {{dlo, minBalance3 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
        ...
        SECTION("missing buying line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            ...
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34a, {{dlo, minBalance3 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
            ...
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                     {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
            ...
            "first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 200}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 200}, {cur3, 20}, {cur4, 0}}},
                 {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 200}, {cur4, 10}}},
                 {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "first exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
        ...
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 1}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 1}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 78}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
            ...
            "second exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 1}, {cur4, 0}}},
                 {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 2}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 78}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 38}, {cur3, 1}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 1}}},
                 {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 4}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 76}, {cur2, 2}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 38}, {cur3, 1}, {cur4, 0}}},
                 {mm34a, {{dlo, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 1}, {cur4, 0}}},
                 {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 18}, {cur4, 1}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 199}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 199}, {cur3, 20}, {cur4, 0}}},
                 {mm23b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 199}, {cur4, 10}}},
                 {mm34b, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    ...
    SECTION("path payment takes all offers, multiple offers per exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 45}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 28}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 17}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
        });
        for_versions(3, 9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance5 - 5 * txfee}, {cur1, 45}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 28}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 17}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment takes all offers, multiple offers per exchange V10")
    {
        ...
        for_versions_from(10, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance5 - 5 * txfee}, {cur1, 59}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 35}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 23}, {cur4, 0}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 16}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment takes best offers, multiple offers per exchange")
    {
        ...
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 16}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance5 - 5 * txfee}, {cur1, 29}, {cur2, 8}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 20}, {cur3, 4}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 13}, {cur4, 2}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 8}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment takes best offers, multiple offers per exchange V10")
    {
        ...
        for_versions_from(10, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 7}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{dlo, minBalance5 - 5 * txfee}, {cur1, 51}, {cur2, 4}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 31}, {cur3, 2}, {cur4, 0}}},
                 {mm34, {{dlo, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 21}, {cur4, 1}}},
                 {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 15}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment with rounding errors")
    {
        ...
        auto sellerOffer = market.requireChangesWithOffer({}, [&] {
            return market.addOffer(seller, {cny, dlo, price, 145000000});
        });
        ...
        for_versions_to(2, *app, [&] {
            ...
            REQUIRE_THROWS_AS(market.requireChanges(
                                  {},
                                  [&] {
                                      source.pay(destination, dlo, 1382068965,
                                                 cny, 20000000, path);
                                  }),
                              ex_PATH_PAYMENT_STRICT_RECEIVE_TOO_FEW_OFFERS);
        });
        for_versions_from(3, *app, [&] {
            auto sellerOfferRemaining =
                OfferState{cny, dlo, price, 145000000 - 20000000};
            auto actual = std::vector<ClaimOfferAtom>{};
            market.requireChanges(
                {{sellerOffer.key, sellerOfferRemaining}}, [&] {
                    actual = source
                                 .pay(destination, dlo, 1382068965, cny,
            ...
            REQUIRE(actual == expected);
            market.requireBalances(
                {{source, {{dlo, 1989999000 - 100 - 1379310345}}},
            ...
            "offers trail")
    {
        ...
        auto path = std::vector<Asset>{dlo, usd};
        ...
        for_versions_to(2, *app, [&] {
            ...
            auto usdCurOffer = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(mm,
                                       {usd, dlo, Price{1, 2}, 2 * offerSize});
            });

            auto res = source.pay(destination, dlo, 8 * paymentToReceive, idr,
    ...
    SECTION("path payment with cycle")
    {
        for_all_versions(*app, [&] {
            ...
            // always half - so sender buys 8 times as much DLO as he/she
            // sells (arbitrage).
            // Second cycle involves 3 transaction in which buying price is
            // always two - so sender buys 8 times as much DLO as he/she
            ...
            auto assets = std::deque<Asset>{dlo, usd, idr};
    ...
    SECTION("liabilities")
    {
        SECTION("cannot pay balance below selling liabilities")
        {
            ...
            auto offer = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(source, {cur1, dlo, Price{1, 1}, 50});
            });
            auto o2 = market.requireChangesWithOffer({}, [&] {
    ...
    SECTION("crossed sponsored offers")
    {
        ...
        SECTION("offer sponsor is source or dest account")
        {
            ...
            for_versions_from(14, *app, [&]() {
                ...
                {
                    runTest(usd, dlo, true);
                }

                SECTION("source, non-native for native")
                {
                    runTest(dlo, usd, true);
                }
                ...
                {
                    runTest(usd, dlo, false);
                }

                SECTION("dest, non-native for native")
                {
                    runTest(dlo, usd, false);
                }
            });
        }
            ...
            "for source")
    {
        ...
        for_versions_from(14, *app, [&]() {
...

            {
                auto tx = transactionFrameFromOps(
                    ...
                    {payor.op(beginSponsoringFutureReserves(mm)),
                     mm.op(manageOffer(0, usd, dlo, Price{1, 1}, 10000)),
            ...
            REQUIRE_THROWS_AS(payor.pay(payee, dlo, 10000),
                              ex_PAYMENT_UNDERFUNDED);

            root.pay(payor, txfee);
            REQUIRE_NOTHROW(payor.pay(payee, dlo, 10000, usd, 10000, {}));
        });
    }
}
...
TEST_CASE("path payment uses all offers in a loop", "[tx][pathpayment]")
{
    ...
    auto useAllOffersInLoop = [&](TestAccount* issuerToDelete) {
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{dlo, minBalance4 - 2 * txfee}, {cur1, 6720}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                {mm12, {{dlo, minBalance3 - 3 * txfee}, {cur1, 1360}, {cur2, 7320}, {cur3, 0}, {cur4, 0}}},
                {mm23, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 680}, {cur3, 7660}, {cur4, 0}}},
                {mm34, {{dlo, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 340}, {cur4, 7830}}},
                {mm41, {{dlo, minBalance3 - 3 * txfee}, {cur1, 7920}, {cur2, 0}, {cur3, 0}, {cur4, 160}}},
                {destination, {{dlo, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    };
