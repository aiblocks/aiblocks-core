...
TEST_CASE("create offer", "[tx][offers]")
{
    ...
    SECTION("create offer errors")
    {
            ...
            for_versions_to(12, *app, [&] {
                REQUIRE_THROWS_AS(
                    market.requireChangesWithOffer(
                        {},
                        [&] {
                            return market.addOffer(a1, {idr, dlo, oneone, 100});
                        }),
            ...
            for_versions_to(12, *app, [&] {
                REQUIRE_THROWS_AS(
                    market.requireChangesWithOffer(
                        {},
                        [&] {
                            return market.addOffer(a1, {dlo, idr, oneone, 100});
                        }),
        ...
        SECTION("create offer without DLO to make for reserve")
        {
            auto a1 = root.create("A", minBalance2);
            a1.changeTrust(idr, trustLineLimit);
            a1.changeTrust(usd, trustLineLimit);
                ...
                for (auto const& p : invalidPrices)
                {
                    REQUIRE_THROWS_AS(
                        market.requireChangesWithOffer(
                            {},
                            [&] {
                                return market.addOffer(a, {dlo, idr, p, 150});
                            }),
                        ex_MANAGE_SELL_OFFER_MALFORMED);
                }
            });
    ...
    SECTION("create offer")
    {
        ...
        SECTION("idr -> dlo")
        {
            for_all_versions(*app, [&] {
                market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {dlo, idr, Price{3, 2}, 100});
                });
            });
        }
        ...
        SECTION("dlo -> idr")
        {
            ...
            SECTION("crossing + create")
            {
                ...
                auto a1Offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {idr, dlo, Price{1, 1}, a1IDrs});
                });
                ...
                auto checkCrossed = [&](TestAccount& b1, int64 actualPayment,
                    ...
                    auto b1Offer =
                        market.addOffer(b1, {dlo, idr, oneone, offerAmount},
                                        OfferState::DELETED);
                    market.requireBalances(
                        {{a1,
                          {{dlo, a1Before + actualPayment},
                           {idr, trustLineBalance - actualPayment}}},
                         {b1,
                          {{dlo, b1Before - txfee - actualPayment},
                           {idr, actualPayment}}}});
                };
    ...
    SECTION("crossing offers with rounding")
    {
        ...
        auto bidding = OfferState{dlo, idr, bidPrice, bidAmount};
        auto asking = OfferState{idr, dlo, askPrice, askAmount};
        ...
        SECTION("bid before ask uses bid price")
        {
            ...
            for_versions_to(2, *app, [&] {
                ...
                // 4715278
                // (rounding down)
                auto updatedAsking = OfferState{idr, dlo, askPrice, 4715278};
                // rounding error, should be 0
                auto updatedBidding = OfferState{dlo, idr, bidPrice, 1};
            ...
            for_versions(3, 9, *app, [&] {
                ...
                // 4715277
                // (rounding up)
                auto updatedAsking = OfferState{idr, dlo, askPrice, 4715277};
                market.requireChangesWithOffer(
        ...
        SECTION("ask before bid uses ask price")
        {
            for_all_versions(*app, [&] {
                ...
                auto updatedBidding = OfferState{dlo, idr, bidPrice, 61363625};
                market.requireChangesWithOffer(
    ...
    SECTION("updated offers respect reserve")
    {
        ...
        for_versions_to(9, *app, [&] {
            auto offer = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(a1, {dlo, usd, oneone, 110});
            });
            market.requireChangesWithOffer({}, [&] {
                return market.updateOffer(a1, offer.key.offerID,
                                          {dlo, usd, oneone, 111},
    ...
    SECTION("max liabilities")
    {
        SECTION("buying non-native")
        {
            for_versions_from(10, *app, [&] {
                ...
                auto market = TestMarket{*app};
                auto offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {dlo, usd, Price{1, 1}, 1000});
                });
                market.requireChangesWithOffer(
                    {{offer.key, OfferState::DELETED}}, [&] {
                        return market.addOffer(acc2,
                                               {usd, dlo, Price{1, 1}, 1000},
                                               OfferState::DELETED);
                    });
            });
        }
        SECTION("selling non-native")
        {
            for_versions_from(10, *app, [&] {
                ...
                auto market = TestMarket{*app};
                auto offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc2, {usd, dlo, Price{1, 1}, 1000});
                });
                market.requireChangesWithOffer(
                    {{offer.key, OfferState::DELETED}}, [&] {
                        return market.addOffer(acc1,
                                               {dlo, usd, Price{1, 1}, 1000},
                                               OfferState::DELETED);
                    });
            });
        }
    ...
    SECTION("cannot create offer that would lead to excess liabilities")
    {
        SECTION("native selling liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                root.pay(acc1, dlo, 500 + txfee);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {dlo, usd, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                root.pay(acc1, dlo, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {dlo, usd, Price{1, 1}, 500});
                });

                // Test when existing offers
                auto reserve = app->getLedgerManager().getLastReserve();
                root.pay(acc1, dlo, 500 + txfee + reserve);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {dlo, usd, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                root.pay(acc1, dlo, txfee);
        ...
        SECTION("native buying liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                // Test when no existing offers
                root.pay(acc1, dlo, 2 * txfee);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {usd, dlo, Price{1, 1},
                ...
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(
                        acc1, o1.key.offerID,
                        {usd, dlo, Price{1, 1}, INT64_MAX - minBalance - 500});
                });

                // Test when existing offers
                root.pay(acc1, dlo, txfee);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {usd, dlo, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_LINE_FULL);
                root.pay(acc1, dlo, txfee);
                auto o2 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {usd, dlo, Price{1, 1}, 500});
                });
            });
        }
    ...
    SECTION("cannot modify offer that would lead to excess liabilities")
    {
        SECTION("native selling liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                root.pay(acc1, dlo, 500 + txfee);
                auto o1 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {dlo, usd, Price{1, 1}, 250});
                });
                root.pay(acc1, dlo, txfee);
                REQUIRE_THROWS_AS(
                    market.updateOffer(acc1, o1.key.offerID,
                                       {dlo, usd, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                root.pay(acc1, dlo, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(acc1, o1.key.offerID,
                                              {dlo, usd, Price{1, 1}, 500});
                });
        ...
        SECTION("native buying liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                REQUIRE_THROWS_AS(
                    market.updateOffer(
                        acc1, o1.key.offerID,
                        {usd, dlo, Price{1, 1}, INT64_MAX - minBalance + 1}),
                    ex_MANAGE_SELL_OFFER_LINE_FULL);
                root.pay(acc1, dlo, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(
                        acc1, o1.key.offerID,
                        {usd, dlo, Price{1, 1}, INT64_MAX - minBalance});
                });
                ...
                root.pay(acc1, dlo, 2 * txfee);
                auto o2 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {usd, dlo, Price{1, 1}, 250});
                });
                root.pay(acc1, dlo, txfee);
                REQUIRE_THROWS_AS(
                    market.updateOffer(acc1, o2.key.offerID,
                                       {usd, dlo, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_LINE_FULL);
                root.pay(acc1, dlo, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(acc1, o2.key.offerID,
                                              {usd, dlo, Price{1, 1}, 500});
                });
            });
        }
    ...
    SECTION("cannot create unauthorized offer")
    {
        for_all_versions(*app, [&] {
            ...
            REQUIRE_THROWS_AS(market.addOffer(acc1, {idr, dlo, Price{1, 1}, 1}),
                              ex_MANAGE_SELL_OFFER_SELL_NOT_AUTHORIZED);
            REQUIRE_THROWS_AS(market.addOffer(acc1, {dlo, idr, Price{1, 1}, 1}),
    ...
    SECTION("modify offer price with liabilities")
    {
        ...
        SECTION("buying native")
        {
            ...
            TestMarket market(*app);
            auto o1 = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(acc1, {idr, dlo, Price{1, 1}, 500});
            });
            ...
            for_versions_from(10, *app, [&] {
                ...
                SECTION("decrease price")
                {
                    auto o2 = market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(
                            acc1, {usd, dlo, Price{1, 1},
                                   INT64_MAX - minBalance - 500 - txfee});
                    });
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
                        return market.updateOffer(acc1, o1.key.offerID,
                                                  {idr, dlo, Price{1, 2}, 500});
                    });
                }
            });
        }
    ...
    SECTION("modify offer assets with liabilities")
    {
        ...
        for_versions_from(10, *app, [&] {
            ...
            {
                checkModifyAssets(dlo, usd, usd, dlo);
            }
            SECTION("buying native swap assets")
            {
                checkModifyAssets(usd, dlo, dlo, usd);
            }
            ...
            {
                checkModifyAssets(dlo, usd, cur1, usd);
            }
            SECTION("selling native change buying asset")
            {
                checkModifyAssets(dlo, usd, dlo, cur1);
            }
            SECTION("selling native change both assets")
            {
                checkModifyAssets(dlo, usd, cur1, cur2);
            }

            SECTION("buying native change buying asset")
            {
                checkModifyAssets(usd, dlo, usd, cur1);
            }
            SECTION("buying native change selling asset")
            {
                checkModifyAssets(usd, dlo, cur1, dlo);
            }
            SECTION("buying native change both assets")
            {
                checkModifyAssets(usd, dlo, cur1, cur2);
            }
            ...
            {
                checkModifyAssets(idr, usd, dlo, usd);
            }
            SECTION("non-native change buying asset native")
            {
                checkModifyAssets(idr, usd, idr, dlo);
            }
        });
    }
    ...
    SECTION("reserve and liabilities checks")
    {
        SECTION("when creating an offer")
        {
            SECTION("selling native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {dlo, idr, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LOW_RESERVE);
                    root.pay(acc1, reserve + txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {dlo, idr, Price{2, 1}, 501}),
                    ...
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {dlo, idr, Price{2, 1}, 500}),
                        ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {dlo, idr, Price{2, 1}, 499});
                    });
                });
            }
            ...
            SECTION("buying native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    ...
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {idr, dlo, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LOW_RESERVE);
                    root.pay(acc1, reserve + txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {idr, dlo, Price{2, 1}, 501}),
                    ...
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {idr, dlo, Price{2, 1}, 500}),
                        ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {idr, dlo, Price{2, 1}, 499});
                    });
                });
            }
        ...
        SECTION("when modifying an offer")
        {
            SECTION("selling native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    auto offer = market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {dlo, idr, Price{2, 1}, 250});
                    });
                    auto offerID = offer.key.offerID;

                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
                                           {dlo, idr, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LINE_FULL);
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
                                           {dlo, idr, Price{2, 1}, 500}),
                        ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
            ...
            SECTION("buying native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    ...
                    auto offer = market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {idr, dlo, Price{2, 1}, 250});
                    });
                    auto offerID = offer.key.offerID;

                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
                                           {idr, dlo, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LINE_FULL);
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
        ...
        SECTION("offer sponsor is other offer account")
        {
            auto runTest = [&](Asset const& selling, Asset const& buying,
                ...
                SECTION("cross three offers fully with residual")
                {
                    ...
                    else if (buying == dlo || selling == dlo)
                    {
                        bExt = 1;
                    }
    ...
    SECTION("pull sponsored offers when authorization is revoked")
    {
        ...
        auto tx = transactionFrameFromOps(
            ...
            {sponsor.op(beginSponsoringFutureReserves(acc1)),
             acc1.op(manageOffer(0, usd, dlo, Price{1, 1}, 100)),
             acc1.op(manageOffer(0, dlo, usd, Price{2, 1}, 100)),
             acc1.op(manageOffer(0, idr, dlo, Price{1, 1}, 100)),
             acc1.op(endSponsoringFutureReserves())},
            {sponsor});

        uint64_t offerIdUsdDlo = 0;
        uint64_t offerIdDloUsd = 0;
        uint64_t offerIdIdrDlo = 0;
        {
            LedgerTxn ltx(app->getLedgerTxnRoot());
            offerIdUsdDlo = ltx.loadHeader().current().idPool + 1;
            offerIdDloUsd = ltx.loadHeader().current().idPool + 2;
            offerIdIdrDlo = ltx.loadHeader().current().idPool + 3;

            TransactionMeta txm(2);
            REQUIRE(tx->checkValid(ltx, 0, 0, 0));
            REQUIRE(tx->apply(*app, ltx, txm));

            REQUIRE(loadOffer(ltx, acc1.getPublicKey(), offerIdUsdDlo));
            REQUIRE(loadOffer(ltx, acc1.getPublicKey(), offerIdDloUsd));
            REQUIRE(loadOffer(ltx, acc1.getPublicKey(), offerIdIdrDlo));
