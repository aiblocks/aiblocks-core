...
The `get` and `put` commands define subprocesses that `aiblocks-core` will
...
called `s3aiblocks` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3aiblocks]
get="curl http://history.aiblocks.io/{0} -o {1}"
put="aws s3 cp {0} s3://history.aiblocks.io/{1}"
~~~~

In this example, `aiblocks-core` will use the `curl` command to fetch history and
